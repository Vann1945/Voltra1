  ...
  return <>
    ...
    <div className={`${isOpen ? 'flex' : 'hidden'} fixed inset-x-4 bottom-[calc(72px+env(safe-area-inset-bottom))] z-[210] flex-col gap-2 rounded-2xl border border-parchment-border bg-parchment-raised p-3 shadow-card-float sm:hidden`} role="dialog" aria-modal="true" aria-labelledby="quick-actions-title">
      <div className="flex items-center justify-between border-b border-parchment-border px-1 pb-3">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-terracotta-text">{t('nav.yourSpace')}</p><span id="quick-actions-title" className="text-sm font-bold">{t('nav.quickActions')}</span></div>
      ...
      {user ? <button type="button" onClick={() => closeAnd(() => onNavigate('profile'))} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left hover:bg-ink-900/[0.04]">
        ...
        <span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold">{user.displayName}</span><span className="block text-xs text-ink-900/55">{t('nav.openProfile')}</span></span><ChevronDown size={15} className="-rotate-90 text-ink-900/40" />
      </button> : <button type="button" onClick={() => closeAnd(onOpenAuth)} className={`${getButtonClasses('primary', 'md')} w-full`}><LogIn size={16} /> {t('nav.signIn')}</button>}
      <button type="button" onClick={() => closeAnd(() => onNavigate('settings'))} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.04]"><SettingsIcon size={16} /> {t('nav.settings')}</button>
      {user && <button type="button" onClick={() => closeAnd(() => onNavigate('creator'))} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.04]"><TrendingUp size={16} /> {t('nav.creatorDashboard')}</button>}
      {user && <button type="button" onClick={() => closeAnd(onOpenUpload)} className={`${getButtonClasses('secondary', 'md')} w-full`}><Upload size={16} /> {t('nav.publishFull')}</button>}
      {user?.role === 'admin' && <button type="button" onClick={() => closeAnd(() => onNavigate('admin'))} className={`${getButtonClasses('secondary', 'md')} w-full`}><Shield size={16} /> {t('nav.admin')}</button>}
      {user && <button type="button" onClick={() => closeAnd(onLogout)} className="flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-bold text-danger hover:bg-danger/[0.07]"><LogOut size={14} />{t('nav.logOut')}</button>}
    </div>
    ...
    <nav aria-label="Primary navigation" className="sticky top-0 z-[100] border-b border-parchment-border bg-parchment-raised/90 shadow-[0_1px_0_rgba(23,35,41,0.02)] backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-1 rounded-2xl border border-parchment-border bg-parchment px-1.5 py-1.5 sm:flex">
          <button type="button" onClick={() => onNavigate('home')} aria-current={isHome ? 'page' : undefined} className={`rounded-xl px-4 py-2.5 text-sm font-bold transition-colors ${isHome ? 'bg-ink-900 text-paper' : 'text-ink-900/65 hover:bg-ink-900/[0.05]'}`}>{t('nav.explore')}</button>
          <button type="button" onClick={() => onNavigate('library')} aria-current={isLibrary ? 'page' : undefined} className={`rounded-xl px-4 py-2.5 text-sm font-bold transition-colors ${isLibrary ? 'bg-ink-900 text-paper' : 'text-ink-900/65 hover:bg-ink-900/[0.05]'}`}><span className="inline-flex items-center gap-2"><Bookmark size={15} />{t('nav.bookmark')}</span></button>
          <button type="button" onClick={() => onNavigate('settings')} aria-current={isSettings ? 'page' : undefined} className={`rounded-xl px-4 py-2.5 text-sm font-bold transition-colors ${isSettings ? 'bg-ink-900 text-paper' : 'text-ink-900/65 hover:bg-ink-900/[0.05]'}`}><span className="inline-flex items-center gap-2"><SettingsIcon size={15} />{t('nav.settings')}</span></button>
        </div>
        <div className="hidden items-center gap-2 sm:flex">
          {user ? <>
            <button type="button" onClick={onOpenUpload} className={getButtonClasses('primary', 'sm')}><Upload size={15} />{t('nav.publish')}</button>
            {user.role === 'admin' && <button type="button" onClick={() => onNavigate('admin')} className={getButtonClasses('secondary', 'sm')}><Shield size={15} />{t('nav.admin')}</button>}
            <div className="relative" ref={profileMenuRef}>
              ...
              {profileMenuOpen && <div id="profile-menu" role="menu" className="absolute right-0 top-full mt-3 w-[min(320px,calc(100vw-32px))] overflow-hidden rounded-2xl border border-parchment-border bg-parchment-raised p-2 shadow-card-float">
                <div className="flex items-center gap-3 rounded-xl bg-parchment px-3 py-3"><ProfileAvatar photoURL={user.photoURL} displayName={user.displayName} borderValue={user.profileBorder} sizeClassName="h-11 w-11" textSizeClassName="text-sm" /><div className="min-w-0"><p className="truncate text-sm font-bold text-ink-900">{user.displayName}</p><p className="mt-0.5 truncate text-xs text-ink-900/55">{t('nav.yourProfile')}</p></div></div>
                <div className="mt-2 grid gap-1">
                  <button type="button" role="menuitem" onClick={() => goFromProfile('profile')} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.05]"><UserRound size={16} />{t('nav.viewProfile')}</button>
                  <button type="button" role="menuitem" onClick={() => { HAPTIC_PATTERNS.light(); onOpenUpload(); setProfileMenuOpen(false); }} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.05]"><Upload size={16} />{t('nav.publishFull')}</button>
                  <button type="button" role="menuitem" onClick={() => goFromProfile('library')} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.05]"><Bookmark size={16} />{t('nav.bookmark')} <span className="ml-auto text-xs font-medium text-ink-900/45">{t('nav.bookmarksHint')}</span></button>
                  <button type="button" role="menuitem" onClick={() => goFromProfile('settings')} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.05]"><SettingsIcon size={16} />{t('nav.settings')}</button>
                  <button type="button" role="menuitem" onClick={() => goFromProfile('creator')} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.05]"><TrendingUp size={16} />{t('nav.creatorDashboard')}</button>
                </div>
                {user.role === 'admin' && <button type="button" role="menuitem" onClick={() => goFromProfile('admin')} className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold hover:bg-ink-900/[0.05]"><Shield size={16} />{t('nav.admin')}</button>}
                <button type="button" role="menuitem" onClick={() => { logout(); setProfileMenuOpen(false); }} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl px-3 py-3 text-sm font-bold text-danger hover:bg-danger/[0.07]"><LogOut size={16} />{t('nav.logOut')}</button>
              </div>}
            </div>
          </> : <button type="button" onClick={onOpenAuth} className={getButtonClasses('primary', 'sm')}><LogIn size={15} />{t('nav.signIn')}</button>}
        </div>
      </div>
    </nav>

...
export function AddonDetail({ addonId, addons, userLikes, onToggleLike, onRequireAuth, onNavigate }: AddonDetailProps) {
  ...
  return (
    <article className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="overflow-hidden rounded-[2.5rem] border border-white/5 bg-zinc-900/40 backdrop-blur-3xl shadow-[0_16px_48px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] ">
          ...
          <div className="mb-8 flex flex-wrap items-start justify-between gap-6">
            <div>
              <div className="mb-6 flex flex-wrap items-center gap-3">
                <span className="inline-flex items-center rounded-full bg-black/40 backdrop-blur-md px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-widest text-white border border-white/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_4px_rgba(0,0,0,0.5)]">
                  {addon.category}
                </span>
                {addon.status === 'pending' && (
                  <span className="inline-flex items-center rounded-full bg-amber-500/40 backdrop-blur-md px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-widest text-amber-50 border border-amber-500/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_4px_rgba(0,0,0,0.5)]">
                    Pending Approval
                  </span>
                )}
                {addon.status === 'rejected' && (
                  <span className="inline-flex items-center rounded-full bg-red-500/40 backdrop-blur-md px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-widest text-red-50 border border-red-500/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_4px_rgba(0,0,0,0.5)]">
                    Rejected
                  </span>
                )}
              ...
              <div className="mt-8 flex flex-wrap items-center gap-6 text-sm text-zinc-400">
                ...
                {addon.averageRating !== undefined && (
                  <div className="flex items-center gap-1.5 text-zinc-200 bg-zinc-800/80 px-3 py-1.5 rounded-full border border-white/5 shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_2px_4px_rgba(0,0,0,0.3)]" aria-label={`Rating: ${addon.averageRating.toFixed(1)} out of 5 stars, from ${addon.ratingCount} reviews`}>
                    <Star size={16} className="fill-amber-400 text-amber-400 drop-shadow-[0_0_4px_rgba(251,191,36,0.6)]" aria-hidden="true" />
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <button
                ...
                className={`flex items-center gap-2 rounded-full border border-transparent px-5 py-3 text-sm font-bold transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 ${
                  isLiked ? 'text-rose-500 bg-rose-500/10 shadow-[inset_0_1px_4px_rgba(0,0,0,0.2)]' : 'text-zinc-400 bg-zinc-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)] hover:bg-zinc-800 hover:text-zinc-200 hover:shadow-[0_2px_8px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.05)]'
                }`}
              >
                <Heart size={20} className={isLiked ? 'fill-rose-500 drop-shadow-[0_0_6px_rgba(244,63,94,0.6)]' : ''} aria-hidden="true" />
              ...
              <button
                ...
                className={`relative overflow-hidden flex items-center gap-2 rounded-full px-8 py-3 text-sm font-bold transition-all hover:scale-105 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50 ${
                  downloadSuccess 
                    ? 'bg-emerald-500 text-black shadow-[0_4px_12px_rgba(16,185,129,0.4),inset_0_1px_0_rgba(255,255,255,0.5)]' 
                    : 'bg-zinc-100 text-black shadow-[0_4px_16px_rgba(255,255,255,0.2),inset_0_1px_0_rgba(255,255,255,1)] hover:bg-white hover:shadow-[0_6px_24px_rgba(255,255,255,0.3)]'
                }`}
              >
                {isDownloading && (
                  <div 
                ...
                <div className="relative z-10 flex items-center gap-2">
                  {isDownloading ? (
                    ...
                    <Download size={20} aria-hidden="true" strokeWidth={2.5} />
                  )}
              ...
              {addon.demoUrl && (
                <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] rounded-3xl p-8 ">
                  <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-3">
                    <ExternalLink size={20} className="text-white" strokeWidth={2.5} />
                    Demo / Preview
                  </h3>
                  <a 
                    href={addon.demoUrl} 
                ...
                <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] rounded-3xl p-8 ">
                  ...
                  <div className="space-y-6">
                    ...
                    {addon.compatibilityNotes && (
                      <div>
                        <h4 className="text-xs font-extrabold text-zinc-500 uppercase tracking-widest mb-2">Compatibility</h4>
                        <p className="text-sm text-zinc-300 font-medium">{addon.compatibilityNotes}</p>
                      </div>
                    )}
                    {addon.changelog && (
                      <div>
                        <h4 className="text-xs font-extrabold text-zinc-500 uppercase tracking-widest mb-2">Changelog</h4>
                        <p className="text-sm text-zinc-300 font-medium whitespace-pre-wrap">{addon.changelog}</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="mt-20">
        ...
        <div className="mb-12 rounded-[2.5rem] border border-white/5 bg-zinc-900/40 backdrop-blur-3xl shadow-[0_16px_48px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] p-8 sm:p-12 ">
          ...
          <form onSubmit={handleSubmitReview}>
            ...
            <div className="mb-8">
              <label className="block text-sm font-extrabold text-zinc-500 uppercase tracking-widest mb-4">Review</label>
              <textarea
                required
                rows={4}
                value={newReviewText}
        ...
        >
          {reviews.length === 0 ? (
            <div className="md:col-span-2 rounded-[2.5rem] border border-white/5 bg-zinc-900/40 backdrop-blur-3xl shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] py-20 text-center ">
              <MessageSquare size={40} strokeWidth={1.5} className="mx-auto mb-6 text-zinc-600" />

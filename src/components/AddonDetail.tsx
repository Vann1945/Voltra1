  ...
  if (!addon) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-white text-balance">Add-on not found</h2>
  ...
  return (
    <article className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      <button 
        ...
        className="mb-8 flex items-center gap-2 text-sm font-medium text-zinc-500 hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 rounded-lg p-1 -ms-1 active:scale-[0.96]"
      >
          ...
          {images.length > 1 && (
            <>
              <button 
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 p-4 rounded-full bg-black/40 backdrop-blur-md text-white opacity-0 group-hover/carousel:opacity-100 transition hover:bg-black/60 hover:scale-110 active:scale-[0.96] shadow-[0_4px_16px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.2)]    focus-visible:opacity-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/50 "
              >
              ...
              <button 
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 p-4 rounded-full bg-black/40 backdrop-blur-md text-white opacity-0 group-hover/carousel:opacity-100 transition hover:bg-black/60 hover:scale-110 active:scale-[0.96] shadow-[0_4px_16px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.2)]    focus-visible:opacity-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/50 "
              >
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2" role="tablist" aria-label="Image gallery">
                {images.map((_, idx) => (
                  <button 
                    ...
                    className={`h-2 rounded-full transition focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-black ${idx === currentImageIndex ? 'w-8 bg-white shadow-[0_0_8px_rgba(255,255,255,0.8)]' : 'w-2 bg-white/40 hover:bg-white/80'}`}
                  />
                ))}
              </div>
            </>
        ...
        <AnimatePresence>
          {isImageExpanded && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 ">
              <button 
                className="absolute top-6 right-6 p-4 rounded-full bg-white/10 text-white hover:bg-white/20 transition hover:scale-110 active:scale-[0.96] hover:rotate-90    z-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/50 "
                ...
                {images.length > 1 && (
                  <>
                    <button 
                      onClick={prevImage}
                      aria-label="Previous image"
                      className="absolute left-4 sm:left-12 top-1/2 -translate-y-1/2 p-4 sm:p-5 rounded-full bg-white/10 text-white transition hover:bg-white/20 hover:scale-110 active:scale-[0.96]     focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/50 "
                    >
                      <ChevronLeft size={32} />
                    </button>
                    <button 
                      onClick={nextImage}
                      aria-label="Next image"
                      className="absolute right-4 sm:right-12 top-1/2 -translate-y-1/2 p-4 sm:p-5 rounded-full bg-white/10 text-white transition hover:bg-white/20 hover:scale-110 active:scale-[0.96]     focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/50 "
                    >
        ...
        <div className="p-8 sm:p-12">
          <div className="mb-8 flex flex-wrap items-start justify-between gap-6">
            <div>
              ...
              <h1 className="text-5xl font-extrabold tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white to-zinc-400 sm:text-6xl text-balance">{addon.title}</h1>
              
              <div className="mt-8 flex flex-wrap items-center gap-6 text-sm text-zinc-400">
                <button 
                  className="flex items-center gap-3 hover:text-white transition-colors cursor-pointer group/author    rounded-lg pe-2 active:scale-[0.96] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/50 "
                  onClick={handleAuthorClick}
                  aria-label={`View ${addon.authorName}'s profile`}
                >
                  <div className={`h-12 w-12 shrink-0 overflow-hidden rounded-full bg-zinc-800 shadow-[inset_0_1px_4px_rgba(0,0,0,0.5),0_2px_4px_rgba(0,0,0,0.3)] flex items-center justify-center transition group-hover/author:border-white/30 border border-white/5 ${getBorderClass(authorBorder)}`} aria-hidden="true">
                    {authorPhoto ? (
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className={`flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-medium transition-colors active:scale-[0.96]  ${
              ...
              <button
                ...
                className={`relative overflow-hidden flex items-center gap-2 rounded-xl px-8 py-3 text-sm font-medium transition-colors active:scale-[0.96]  ${
                  downloadSuccess 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 prose prose-invert max-w-none">
              <h3 className="text-2xl font-bold text-white mb-6 tracking-wide text-balance">Description</h3>
              <p className="whitespace-pre-wrap text-zinc-400 leading-relaxed font-medium text-pretty">{addon.description}</p>
            </div>
            ...
            <div className="space-y-8">
              {addon.demoUrl && (
                <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] rounded-3xl p-8 ">
                  <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-3 text-balance">
                  ...
                  <a 
                    ...
                    className="inline-flex items-center gap-2 text-sm font-bold text-white bg-white/10 hover:bg-white/20 transition px-4 py-2 rounded-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.2)]"
                  >
                    View Demo
              ...
              {(addon.versionHistory || addon.compatibilityNotes || addon.changelog) && (
                <div className="bg-zinc-900/50 border border-white/10 rounded-2xl p-6">
                  <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-3 text-balance">
                  ...
                  <div className="space-y-6">
                    {addon.versionHistory && (
                      <div>
                        <h4 className="text-xs font-extrabold text-zinc-500 uppercase tracking-widest mb-2 text-balance">Version History</h4>
                        <p className="text-sm text-zinc-300 font-medium text-pretty">{addon.versionHistory}</p>
                      </div>
                    )}
                    {addon.compatibilityNotes && (
                      <div>
                        <h4 className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2 text-balance">Compatibility</h4>
                        <p className="text-sm text-zinc-300 text-pretty">{addon.compatibilityNotes}</p>
                      </div>
                    )}
                    {addon.changelog && (
                      <div>
                        <h4 className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2 text-balance">Changelog</h4>
                        <p className="text-sm text-zinc-300 whitespace-pre-wrap text-pretty">{addon.changelog}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {addon.tags && addon.tags.length > 0 && (
                <div>
                  <h3 className="text-xs font-extrabold text-zinc-500 uppercase tracking-widest mb-4 text-balance">Tags</h3>
      ...
      {/* Reviews Section */}
      <div className="mt-20">
        <h2 className="text-4xl font-extrabold tracking-tighter text-white mb-10 flex items-center gap-4 text-balance">
        ...
        <div className="mb-12 rounded-[2.5rem] border border-white/5 bg-zinc-900/40 backdrop-blur-3xl shadow-[0_16px_48px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] p-8 sm:p-12 ">
          <h3 className="text-2xl font-bold text-white mb-8 tracking-wide text-balance">Write a Review</h3>
          <form onSubmit={handleSubmitReview}>
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                className="block w-full rounded-2xl border border-transparent bg-zinc-950/80 shadow-[inset_0_2px_8px_rgba(0,0,0,0.5)] px-6 py-5 text-white placeholder-zinc-600 focus:border-white/20 focus:bg-zinc-900 focus:outline-none transition resize-none font-medium"
            ...
            <div className="flex justify-end">
              <button
                ...
                className="rounded-full bg-white px-10 py-4 text-sm font-extrabold text-black hover:bg-zinc-200 disabled:opacity-50 transition hover:scale-105 active:scale-[0.96] hover:shadow-[0_4px_16px_rgba(255,255,255,0.2)]    focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/50 "
              >
        ...
        >
          {reviews.length === 0 ? (
            <div className="md:col-span-2 rounded-[2.5rem] border border-white/5 bg-zinc-900/40 backdrop-blur-3xl shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] py-20 text-center ">
              ...
              <p className="text-zinc-400 font-bold tracking-wide text-lg text-pretty">No reviews yet. Be the first to review!</p>
            </div>
          ) : (
            reviews.map((review) => {
              ...
              return (
                <motion.div 
                  ...
                  className="group rounded-[2.5rem] border border-white/5 bg-zinc-900/60 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] p-8 transition hover:bg-zinc-800 flex flex-col h-full"
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-4">
                      <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ${authorColor} font-extrabold shadow-[inset_0_1px_4px_rgba(0,0,0,0.2),0_2px_4px_rgba(0,0,0,0.3)] transition`}>
                        {review.userName.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="text-lg font-bold text-white tracking-tight text-pretty">{review.userName}</p>
                        <p className="text-[10px] font-extrabold uppercase tracking-widest text-zinc-500 mt-1 text-pretty">{formatDistanceToNow(new Date(review.createdAt), { addSuffix: true })}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <p className="text-zinc-300 whitespace-pre-wrap leading-relaxed font-medium text-sm flex-grow text-pretty">{review.text}</p>
                </motion.div>
              );
            })
          )}

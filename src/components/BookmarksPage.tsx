...
import { Heart, LockKeyhole, Bookmark } from 'lucide-react';
...
export function BookmarksPage({ addons, userLikes, userBookmarks, onToggleLike, onToggleBookmark, onRequireAuth, onNavigate }: BookmarksPageProps) {
  ...
  return <section className="min-h-[calc(100dvh-64px)] bg-parchment pb-32" aria-label="Saved projects">
    <div className="border-b border-parchment-border bg-parchment-raised">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        ...
        <div className="mt-4 flex flex-wrap items-end justify-between gap-6"><div><h1 className="text-4xl font-bold tracking-[-0.04em] text-ink-900 sm:text-5xl">Saved projects</h1><p className="mt-4 max-w-2xl text-base leading-7 text-ink-900/60">Keep saved projects and liked finds together, ready whenever you want to return.</p></div><div className="hidden rounded-2xl border border-parchment-border bg-parchment px-4 py-3 text-right sm:block"><p className="text-xs font-bold uppercase tracking-[0.14em] text-ink-900/45">Total collection</p><p className="mt-1 text-xl font-bold text-ink-900">{savedAddons.length + likedAddons.length}</p></div></div>
      </div>
    </div>
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      {!user ? <div className="rounded-2xl border border-parchment-border bg-parchment-raised px-6 py-20 text-center shadow-card"><LockKeyhole size={32} className="mx-auto text-ink-900/35" /><h2 className="mt-4 text-xl font-bold text-ink-900">Sign in to open your saved projects</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-ink-900/55">Your saved and liked projects stay private to your account and sync across devices.</p><button type="button" onClick={onRequireAuth} className={`mt-6 ${getButtonClasses('primary', 'md')}`}>Sign in</button></div> : <>
